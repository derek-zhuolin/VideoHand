---
name: videohand
description: 把一段文字做成纸面马克笔手绘风格的小视频（videohand）。逐句按语义选画面卡（63 张可选，相邻不重样），rough.js 笔画原语建帧，安全区 + 三画幅自适应（9:16 / 16:9 / 1:1），可选配音与词级手绘字幕，渲染前过五道闸。当用户要做手绘 / 涂鸦 / 白板 / sketch 风格的解说短片、观点片、上新片、知识拆解、教程视频时用它，或用户直接点名 videohand / handdrawn 时用它。
---
...
`playground/index.html` 是 63 张卡的动图墙，每格跑的是那张卡的**真实代码** ——
所以它同时是 63 张卡的冒烟测试，哪张写错了那格会红着报错。
...
> 「这是给 **<谁>** 看的 **<什么形状>** 的片，调子 **<稳 / 冲 / 轻>**，画幅 **<9:16 / 16:9 / 1:1>**，配色 **<纸绿默认 / 砖红 / 靛蓝>**。」

写在 STORYBOARD 开头。这句话决定后面的选卡倾向（教程片偏 D/H，观点片偏 B/E/G，
上新片偏 B/F）、能量分布和配色（预设见 `references/palette.md`）——
没有它，选卡就退化成「哪张顺手挑哪张」，片子会和上一支长成一个样。
...
| 相邻不重卡 | 相邻两格禁用同一张卡；全片同卡 ≤1 次（≥6 格时 ≤2 次且不相邻） |
| 相邻不重族 | 相邻两格禁用同族卡（卡名不同、版面结构同一个模子也算重）；全片族种类 ≥ ⌈格数/2⌉ |
| 高能配额 | `one-word-explode` / `cross-out-correct` / `title-scribble-reveal` / `torn-paper-reveal` / `explode-parts` 合计 ≤2 处且不相邻 |
| 呼吸帧 | ≥1 处低能镜头。`quote-bracket-hold` 和 `smudge-focus` 都算——别每支都拿同一张，也别每支都钉在落版前一格 |
| 转场 | 只用手绘转场（8 种见 `references/transitions.md`），**每条缝都写进 STORYBOARD（硬切写 `transition: hard-cut`）**，相邻两条缝不许同一种（硬切除外），非硬切种类 ≥ ⌈非硬切数/2⌉ |
| 跨片查重 | scene-lint 自动记账（`.ledger.json`）：与上一支用卡重合 >40%、呼吸帧连续两支钉在同一位置，都会被点名。63 张卡是拿来轮换的，不是摆着看的 |
...
**每条缝都要有 `## Seam N-M`，硬切也写（`transition: hard-cut`）**，
非硬切的再记 SEAM SEED。硬切只是帧里不写 seam 代码，STORYBOARD 里不能缺席——
缺席的缝 scene-lint 看不见，转场种类那条规则就永远空过（实测三支片全中）。
...
**纸绿是默认，不是唯一。** 读题（第 0 步）判断调性明显不是绿时，换
`references/palette.md` 里的预设（炭墨砖红 / 靛蓝米纸，对比度都已算过）——
只换 token 块，卡一张不用碰。
