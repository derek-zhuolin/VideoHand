...
![63 张画面卡里精选的 16 张](docs/assets/wall.gif)

↑ 这是 63 张里的 16 张。全部 63 张去**[在线 63 格墙](https://derek-zhuolin.github.io/VideoHand/)**
直接逛，不用装任何东西 —— 同时循环播，顶部可切 9:16 / 16:9 / 1:1 和中英文，
四个滑杆实时调笔触。

装完本地也有同一面墙：`npx github:derek-zhuolin/VideoHand playground`。

每一格跑的都是那张卡的**真实代码**，所以这面墙同时也是 63 张卡的冒烟测试：
哪张写错了，那一格会红着报错。
...
这个 skill 把那个本能拆成了 63 张画面卡，逐句去挑：
...
很厚（`hw-cards.js` 光 63 张卡就上千行），却不占 Claude 的预算。这也是为什么
...
颜色只写在**一处 token 块**里，改色不用碰 63 张卡。为什么马克笔绿只能画线不能写字
...
│   ├── hw-cards.js             # 63 张卡的 build 函数（唯一真源）
...
├── templates/                  # 帧脚手架 + 字幕皮肤
├── playground/index.html       # 63 格动图墙（生成物，不进包）
...
| 有句话在 63 张卡里找不到对应形状 | **想要一张新画面卡** —— 说清楚那句话「是什么形状」 |
